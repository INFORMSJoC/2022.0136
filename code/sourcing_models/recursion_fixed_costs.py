...
def vf_update(state, vf, demand_prob, actions):
    ...
    for qe, qr in actions:
        ...
        # We should order at least as much as necessary
        # to replenish the current inventory shortage
        incoming = sum(pipeline) if lr > 2 else qr
        skip = False
        if qr < - (ip_e + incoming):
            skip = True
        # We will never order so much emergency so that we end up
        # carrying units over
        # if qe > max(d_max - (state[0] + state[1]), 0.):
            # skip = True
            # print(f'rejected qe {qe} in state {state}')
        if skip:
            continue

        for dem in range(int(d_min), int(d_max) + 1):
            ipe_new = ip_e - dem
    ...
    # if best_action:
        # if best_action[0] > max(d_max - (state[0] + state[1]), 0):
            # print(f'qe {best_action[0]} is best action for state {state}')
...
def main():
    ...
    # This is not quite true for low service levels - only for their instances
    actions_ = list(product(range(int(d_max) + 1), range(int(d_max*lr) + 1)))
    actions = List()
